...
public class ShopSearchServiceApplicationTests {
    ...
    public void insertData() throws IOException, SolrServerException {
        for (int i = 0; i < 10; i++) {
            ...
            if( i==5 ){
                document.addField("gname","华为华为手机");
            }else {
                document.addField("gname","华为手机");
            }

            document.addField("ginfo","性价比很高的国产手机，国产手机中的战斗机"+i);
            document.addField("gprice",9090+i);
            document.addField("gsave" ,1000+i);
            document.addField("gimages","http://baidu.com");
    ...
    @Test
    public  void deleteDate() throws IOException, SolrServerException {
       // solrClient.deleteById("1");
        solrClient.deleteByQuery("gname:华为"); //选择删除
        solrClient.commit();
}
@Test
